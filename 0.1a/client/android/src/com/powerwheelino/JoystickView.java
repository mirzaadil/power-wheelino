...
public class JoystickView extends View {
	...
	String TAG = "Powerwheelino_Joystick";
	...
	protected void onLayout(boolean changed, int left, int top, int right,
		...
		//absolute position from bottom right corner
		cX = dimX - 125;
		cY = dimY - 125;
		...
		movementRadius = 70; // Math.min(cX, cY) - handleInnerBoundaries;
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		if (settings.getBoolean("debugMode", false)) {
			...
			int baseY = (int) (touchY < 0 ? cY + handleRadius : cY
					- handleRadius);
			canvas.drawText(
					String.format("%s (%.0f,%.0f)", "x,y", touchX, touchY),
...
//		Log.d(TAG,
//				"Touch X: " + String.valueOf(touchX) + " Touch Y:"
